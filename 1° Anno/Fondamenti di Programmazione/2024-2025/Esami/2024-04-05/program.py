...
def func3(S1, S2):
    ...
    for stringa in S1:
        ...
        for string in S2:
            if string[0] == stringa:
                lista.append(string)
        risultato[stringa] = lista
    risultato = dict(sorted(risultato.items()))
    return risultato
...
def func5(file_txt, K, M):
    with open(file_txt, mode="r", encoding="utf8") as F:
        righe = F.readlines()
        matrice = []
        for riga in righe:
...
def ex1(dirname, necessary, forbidden):
    ...
    for nome in os.listdir(dirname):
        ...
        if os.path.isdir(fullname):
            risultato.extend(ex1(fullname, necessary, forbidden))
        elif nome.endswith((".txt")):
            with open(fullname, mode = "r", encoding = "utf8") as F:
                testo = F.read()
                testo_splittato = testo.split()
                tutte_necessarie = all(parola in testo_splittato for parola in necessary)
                nessuna_forbidden = all(parola not in testo_splittato for parola in forbidden)
                if tutte_necessarie and nessuna_forbidden:
                    risultato.append(fullname)
    risultato = sorted(risultato, key=lambda f: (f.count('/'), -ord(f[0])), reverse = True)
    return risultato
