...
def func2(D1:dict[str,list[int]], D2: dict[int,str]) -> dict[str, list[str]]:
    ...
    for chiave, lista_interi in D1.items():
        valori = []
        for num in lista_interi:
            if num in D2:                 # se il numero ha una corrispondenza in D2
                valori.append(D2[num])
        if valori:  # aggiungo solo se la lista non è vuota
            valori.sort(key=lambda x: (-len(x), x))  # ordinamento come richiesto
...
"""
def func3(parole: list[str], k:int, S:set[str]) :
...
"""
def func4(path_in: str, path_out: str, K: int) -> dict[str, set[tuple[int,int]]]:
    dizionario = {}
    
    # leggo tutte le righe del file
    with open(path_in, "r", encoding="utf8") as f:
        righe = [riga.strip().split() for riga in f.readlines()]
    
    # creo un insieme di tutte le parole presenti nel file
    parole_set = set()
    for riga in righe:
        parole_set.update(riga)
    
    # costruisco il dizionario
    for parola in parole_set:
        apparizioni = set()
        for i, riga in enumerate(righe):
            count = riga.count(parola)
            if count > 0:
                apparizioni.add((i, count))
        if len(apparizioni) >= K:
            dizionario[parola] = apparizioni
    
    # scrivo su file il risultato
    with open(path_out, "w", encoding="utf8") as f:
        for parola in sorted(dizionario.keys()):
            num_righe = len(dizionario[parola])
            tot_apparizioni = sum(c for _, c in dizionario[parola])
            f.write(f"{parola} {num_righe} {tot_apparizioni}\n")
...
def ex2(path : str) -> dict[str, dict[str,int]]:
    dir_corr: dict = {}
    sottodizionario: dict = {}
    for nome in os.listdir(path):
        if nome[0] in '_.': continue
        fullname = path + '/' + nome
        if os.path.isfile(fullname):
            estensione = os.path.splitext(nome)[1][1:]
            if estensione not in sottodizionario:
                sottodizionario[estensione] = 0
            sottodizionario[estensione] += 1
        elif os.path.isdir(fullname):
            risultato = ex2(fullname)
            dir_corr.update(risultato)
    dir_corr[path] = sottodizionario
    return dir_corr
