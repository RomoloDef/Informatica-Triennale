...
def func1(int_list, keys):
    ...
    for n in int_list:
        for k in keys:
            if n % k == 0:
                dizionario[k].append(n)
    for k in dizionario:
        dizionario[k].sort(reverse=True)
    return dizionario
    pass
...
def func2(L0, L1):
    nuova_stringa = ""
    lista = list(zip(L0, L1))
    for tupla in lista:
        nuova_stringa += tupla[0] * tupla[1]
...
def func3(string_list1, string_list2):
    ...
    for stringa in string_list1:
        for s in string_list2:
            if (s in stringa) or (s[::-1] in stringa):
    ...
    lista = sorted(lista, key = criterio_di_ordinamento, reverse = False)
    return lista
...
def func4(D):
    ...
    criterio_di_ordinamento = lambda x: (-len(x[1]), x[0])
    lista = sorted(lista, key = criterio_di_ordinamento, reverse = False)
    return lista
    pass
    ...
    for y in range(altezza):
        ...
        while x < larghezza:
            if img[y][x] != (0, 0, 0):
                colore = img[y][x]
                x_start = x
                while x < larghezza and img[y][x] == colore:
                    x += 1
                lunghezza = x - x_start
                segmenti.append((colore, lunghezza))
            else:
                x += 1

    # rimuovo duplicati prendendo solo il segmento più lungo per ogni colore
    segmenti_filtrati = {}
    for colore, lunghezza in segmenti:
        if colore not in segmenti_filtrati or lunghezza > segmenti_filtrati[colore]:
            segmenti_filtrati[colore] = lunghezza

    # trasformo in lista di tuple e ordino
    lista_finale = list(segmenti_filtrati.items())
    criterio_ordinamento = lambda elem: (-elem[1], elem[0][2], elem[0][1], elem[0][0])
    lista_finale = sorted(lista_finale, key=criterio_ordinamento)

    # restituisco solo i colori
    colori = []
    for colore, lunghezza in lista_finale:
...
def ex1(L):
    ...
    for p in primo:
        for r in resto:
            risultato.append(p + r)
    criterio_ordinamento = lambda x: (x)
    risultato = sorted(risultato, key = criterio_ordinamento, reverse = False)
    return risultato
...
def ex2(root):
    if root is None:
        return 0
    sinistra = ex2(root.left)
    destra = ex2(root.right)
    root.value += sinistra + destra
    ...
    return valore_dispari, valore_pari
