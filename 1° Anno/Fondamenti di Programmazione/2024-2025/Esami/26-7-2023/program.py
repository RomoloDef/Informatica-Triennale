...
def func1(diz1, diz2):
    ...
    chiavi1 = diz1.keys()
    chiavi2 = diz2.keys()
    chiavi_comuni = chiavi1 & chiavi2
    for chiave in chiavi_comuni:
        lista1 = diz1[chiave]
        lista2 = diz2[chiave]
        unici = set(lista1) ^ set(lista2)
        ordinamento = lambda x: (-len(x), x)
        unici = sorted(unici, key = ordinamento)
        risultato[chiave] = unici
    return risultato
...
def func2(text):
    testo = text.lower().split()
    parole = {}
    for parola in testo:
        if parola[0] in parole:
            parole[parola[0]] += 1
        else:
            parole[parola[0]] = 1 
    return parole
...
def func3(textfile_in, textfile_out):
    with open(textfile_in, mode = "r", encoding = "utf8") as F:
        lista_di_numeri = []
        testo = F.read().split()
        for stringa in testo:
            lista_di_numeri.append(float(stringa))
        lista = []
        for num in lista_di_numeri:
            if isinstance(num, float) and num.is_integer():
                lista.append(int(num))   
            else:
                lista.append(num)
        lista = sorted(lista, key = abs, reverse = True)
    return len(lista)

    with open(textfile_out, mode = "w", encoding = "utf8") as F:
        for numero in lista:
            F.write(numero + ',' + ' ')
...
def func4(input_filename):
    with open(input_filename, mode = "r", encoding = "utf8") as F:
        ...
        for i in range(c):
            nuova_riga = []
            for j in range (n):
                nuova_riga.append(matrice[n - 1 - j][c - 1 - i])
            matrice_riflessa.append(nuova_riga)
        return matrice_riflessa
    pass
