...
def func2(parole):
    ...
    for parola in parole:
        ...
        for iniziale in risultato:
            risultato[iniziale] = sorted(risultato[iniziale], key=lambda x: (len(x), -ord(x[0])))
...
def func4(path_in : str, path_out : str, K : int) -> dict[str, list[str]]:
    with open(path_in, mode = "r", encoding = "utf8") as file:
        risultato: dict = {}
        conteggio: dict = {}
        righe_parole: dict = {}
        ...
        for indice, riga in enumerate(righe):
            riga = riga.split()
            for stringa in riga:
                if stringa in conteggio:
                        conteggio[stringa] += 1
                else:
                        conteggio[stringa] = 1
                
            for stringa in list(set(riga)):
                if stringa in righe_parole:
                    righe_parole[stringa].append(indice)
                else:
                    righe_parole[stringa] = [indice]        
                
        for chiave, valore in righe_parole.items():
            if conteggio[chiave] > K:
...
path_in  = f"func4/in_1.txt"
path_out = f"func4/out_1.txt"
K  = 2
...
def ex2(path : str, lista_estensioni : list[str]) -> dict[str, list[str]]:
    ...
    for nome in os.listdir(path):
        ...
        if os.path.isdir(fullname):
            trovato = ex2(fullname, lista_estensioni)
            for est, dirs in trovato.items():
                risultato.setdefault(est, set()).update(dirs)
        elif os.path.isfile(fullname):
            for estensione in lista_estensioni:
                if nome.endswith('.' + estensione):
                    risultato.setdefault(estensione, set()).add(path)
...
# directory  = 'ex2/A'
# extensions = ["txt", "pdf", "png", "gif"]
# print(ex2(directory, extensions))
