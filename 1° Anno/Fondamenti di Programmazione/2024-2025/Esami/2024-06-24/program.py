...
def func2(L0, L1):
    risultato = ""
    for parola, n in zip(L0, L1):
        ripetizione = parola
        for i in range(1, n):
            ripetizione += parola[1:]
        risultato += ripetizione
    return risultato
...
def func3(file_in):
    ...
    for riga in righe:
        ...
        for x in parti:
            numeri.append(int(x))
        righe_splittate.append(numeri)

    somma_numeri_pari = 0
    for i in range(len(righe_splittate)):
        if i % 2 == 0:
            for numero in righe_splittate[i]:
    ...
    for i in range(len(righe_splittate)):
        if i % 2 != 0:
            for numero in righe_splittate[i]:
                somma_numeri_dispari += numero
    

    return somma_numeri_pari - somma_numeri_dispari
