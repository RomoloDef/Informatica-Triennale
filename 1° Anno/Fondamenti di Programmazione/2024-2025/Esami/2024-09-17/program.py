...
def func3(words, l):
    ...
    for parola in words:
        for word in words:
            if parola != word and parola in word and len(parola) >= l:
                lista.append(parola)
    return set(lista)
    pass
...
def func4(file_in, file_out, length, chars):
    with open(file_in, mode = "r", encoding = "utf8") as F:
        ...
        stringa = ""
        for riga in righe:
            lista_righe.append(riga.strip())
        for frase in lista_righe:
            frase = frase.split(',')
            print(frase)
            for parola in frase:
                stringa += ' ' +  parola
        ...
        for parola in testo:
            for c in parola:
                if len(parola) >= length and c in chars:
                    lista_finale.append(parola)
        finale = list(set(lista_finale))
        ordinamento = lambda el: (-len(el), el)
        lista_parole = sorted(finale, key = ordinamento)
        return lista_parole
    
    with open(file_out, mode = "w", encoding = "utf8") as F:
        F.write(" ".join(lista_parole))
...
def func5(input_png, output_png, m, M):
    ...
    out_img = [[(0,0,0) for _ in range(larghezza)] for _ in range(altezza)]
    ...
    for y in range(altezza):
        ...
        while x < larghezza:
            if img[y][x] != (0, 0, 0):
                ...
                x += 1
    images.save(out_img, output_png)
    return linee_input - linee_output
...
def ex1(chars, l):
    if l == 0:
        return {''}  # palindrome vuota
    if l == 1:
        return set(chars)  # palindrome di un solo carattere
    
    risultato = set()
    sub_palindromi = ex1(chars, l-2)
    
    for c in chars:
        for sub in sub_palindromi:
            risultato.add(c + sub + c)
