...
def func1(embedding_list1, embedding_list2):
    ...
    similitudini = []
    for word1, vec1 in embedding_list1:
        for word2, vec2 in embedding_list2:
            sim = cosine_similarity(vec1, vec2)
            similitudini.append((sim, vec1, vec2))
    massimo = max(similitudini)
    for i in range(len(massimo)-1,-1,-1):
        if massimo[i] == vec1 or massimo[i] == vec2:
            return (word1, word2, massimo[0])
    ...
    for frase in text_corpus:
        ...
        for parola in parole:
            if parola in sentiment_lexicon:
                overall += float(sentiment_lexicon[parola])
    o = round(overall / len(text_corpus), 2)
    top = []
    for frase in text_corpus:
        parole = tokenize_alphabetic(frase)
        valori = []
        for p in parole:
            if p in sentiment_lexicon:
                valori.append(float(sentiment_lexicon[p]))
        if valori:
            massimo = valori[0]
            for v in valori:
                if v > massimo:
                    massimo = v
            top.append((massimo, frase))
    top = top[0]
    dizionario = {'overall_sentiment': o, 'top_positive_sentences': top, 'total_sentences_analyzed': len(text_corpus)}
    return dizionario
    pass
...
def func3(sentence_list, forbidden_words):
    for f in range(len(sentence_list)-1,-1,-1):
        frase = sentence_list[f].lower()
...
def func4(text, n):
    occorrenza = {}
    testo = tokenize_alphabetic(text)
    for parola in testo:
        if parola in occorrenza:
            occorrenza[parola] += 1
        else:
            occorrenza[parola] = 1
    risultato = sorted(occorrenza.items(), key = lambda x: x[1], reverse = True)[:n]
    return risultato
    pass
...
def func5(word_list):
    ...
    for parola in word_list:
        chiave = "".join(sorted(parola.lower()))  
        if chiave in gruppi:
            gruppi[chiave].add(parola)
        else:
            gruppi[chiave] = {parola}
