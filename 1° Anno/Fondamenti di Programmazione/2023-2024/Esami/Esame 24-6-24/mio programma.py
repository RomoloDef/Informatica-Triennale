...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep 13 10:19:00 2024
    ...
    for lista in L:
        for carattere in lista:
            if lista.count(carattere) == 1:
                lista_di_caratteri.append(carattere)
    return lista_di_caratteri
    pass
...
def func3(file_in):
    with open(file_in, 'r', encoding = 'utf-8') as file:
        ...
        for i, line in enumerate(file):
            if i % 2 == 0:
                somma_pari += sum(map(int, line.split()))
            else:
                somma_dispari += sum(map(int, line.split()))
...
def ex1(L):
    if len(L) == 1:
        ...
        return [X+Y for X in L[0] for Y in ex1(L[1:])]
    pass
