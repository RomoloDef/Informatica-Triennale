...
def func1(int_list, n):
    ...
    for chiave, valore in dizionario.items():
        if valore >= n:
            dizionario_finale[chiave] = valore
    return dizionario_finale
...
def func2(dict1, a, b):
    ...
    for chiave in chiavi:
        if chiave[0].lower() >= a.lower() and chiave[0].lower() <= b.lower():
            lista.append(dict1[chiave])
...
def func4(input_filename, output_filename, expr):
    ...
    with open(input_filename, 'r', encoding='utf-8') as f:
        ...
        for numero_riga, riga in enumerate(righe, start = 1):
            if expr.lower() in riga.lower():
                numero_righe += numero_riga
                with open(output_filename, 'w', encoding='utf-8') as f:
                    f.write(riga)
                    num_spazi = riga.count(' ') + riga.count('\t') + riga.count('\n')
                    dizionario[numero_righe] = (len(riga), len(riga.split()), num_spazi)               
    return dizionario
    pass
...
def ex2(root):
    ...
    def conta_nodi(nodo):
        if nodo is None:
            return (0, 0)  # (somma dei valori, numero di nodi che soddisfano la condizione)
        
        # Ricorsivamente troviamo la somma e il numero di nodi per i sottoalberi sinistro e destro
        somma_sx, nodi_sx = conta_nodi(nodo.left)
        somma_dx, nodi_dx = conta_nodi(nodo.right)
        
        # Somma totale dei nodi nei sottoalberi
        somma_sottoalberi = somma_sx + somma_dx
        
        # Verifichiamo se il valore del nodo corrente è maggiore della somma dei suoi sottoalberi
        if nodo.value > somma_sottoalberi:
            return (nodo.value + somma_sottoalberi, nodi_sx + nodi_dx + 1)
        else:
            return (nodo.value + somma_sottoalberi, nodi_sx + nodi_dx)
    
    # Chiamiamo la funzione ricorsiva di supporto a partire dalla radice dell'albero
    _, risultato = conta_nodi(root)
    return risultato
