...
lista_rm = [5, 3, 2, 7]
print(func2(lista_all,lista_rm))
...
def ex1(root, result):
    def somma_profondita(nodo, profondita):
        if nodo is None:
            return
        if profondita >= len(result):
            result.append(0)
        result[profondita] += nodo.value
        somma_profondita(nodo.left, profondita + 1)
        somma_profondita(nodo.right, profondita + 1)
    
    somma_profondita(root, 0)
...
def ex2(dirin, words):
    ...
    for word in words:
        dizionario[word] = 0
    esplora_dirins(dirin, words, dizionario)
    return sorted(dizionario.items(), key = lambda item: (-len(item), len(item)), reverse = True)
        
    
def esplora_dirins(dirin, words, D):
    for nome in os.listdir(dirin):
        ...
        if os.path.isfile(fullname) and fullname.endswith('.txt'):
            ...
            esplora_dirins(fullname, words, D)
