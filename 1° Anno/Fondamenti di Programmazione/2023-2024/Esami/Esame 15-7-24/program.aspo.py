...
output_png sarà come func5/3cime_expected.png e la funzione tornerà
15.  
"""
import images
...
def rotate_square(im, x, y, S):
    ...
    for i, riga in enumerate(new_square):
        for j, pix in enumerate(riga):
            new_square[j][S-i-1] = im[x+i][y+j]
    for i, riga in enumerate(new_square):
        for j, pix in enumerate(riga):
            im[x+i][y+j] = new_square[i][j]
