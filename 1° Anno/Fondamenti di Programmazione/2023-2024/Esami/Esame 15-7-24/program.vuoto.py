...
def func1(L):
    ...
    lista_ordinata = sorted(lista, key=lambda x: (-x[1], x[0]))
...
def func3(L1, L2):
    ...
    for stringa1 in L1:
        for stringa2 in L2:
            if len(stringa1) == len(stringa2) and stringa1 not in L2:
                if stringa1 not in dizionario:
                    dizionario[stringa1] = set()
                dizionario[stringa1].add(stringa2)
    return dizionario
    pass
...
def func4(input_filename, output_filename):
    with open(input_filename, 'r', encoding = 'utf-8') as file:
        testo = file.read()
        conteggio = sum(len(parola) for parola in testo)
        testo = testo.split()
        lunghezza = (len(testo))
        for parola in testo:
            if testo.count(parola) > 1:
                testo.remove(parola)
                testo = list(set(testo))
        gruppi = {}
        for parola in testo:
        ...
        for iniziale in gruppi:
            gruppi[iniziale].sort(reverse=True)
        gruppi_ordinati = sorted(gruppi.items())
    with open(output_filename, 'w', encoding = 'utf-8') as file:
        for iniziale, parole in gruppi_ordinati:
