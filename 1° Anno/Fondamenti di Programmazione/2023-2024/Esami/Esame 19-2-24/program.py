        ...
        for j in list2:
            if i.lower() == j.lower():
                ...
                if lunghezza not in dizionario:
                    dizionario[lunghezza] = {i}
                dizionario[lunghezza].add(i)
                dizionario[lunghezza].add(j)
    return dizionario
    pass
...
def ex1(root):
    def esplora_dizionario(node, profondità, insieme):
        if node is not None:
            if len(insieme) <= profondità:
                insieme.append(set())
            insieme[profondità].add(node.value)
            esplora_dizionario(node.left, profondità+1, insieme)
            esplora_dizionario(node.right, profondità+1, insieme)
        return insieme
    return esplora_dizionario(root, 0 , [])
