...
def func1(a_dict, word):
    ...
    return len(chiavi_da_rimuovere)
    pass
...
def func2(a_string):
    ...
    caratteri_visti = set()
    for c in a_string:
        if c not in caratteri_visti:
            lista_appoggio.append(c)
            caratteri_visti.add(c)
    print(lista_appoggio)
    print(caratteri_visti)
    lista_ordinata = sorted(lista_appoggio, reverse=True)
...
def func3(string_list1, string_list2):
    ...
    for i in range(len(string_list1)):
        stringa_unita = string_list1[i] + string_list2[-(i + 1)]
        stringa_list3.append(stringa_unita)
    stringa_list3.sort(key = lambda x: (len(x), x))                               
    return stringa_list3
    pass
...
def func4(input_file, output_file):
    ...
    print(testo)
    parole = testo.split()
    parole_ordinate = sorted(parole, key = lambda parola: (len(parola), parola.lower(), parola))
    with open(output_file, 'w', encoding = 'utf-8') as f:
        f.write(' '.join(parole_ordinate)+ '\n')
    with open(output_file, 'r', encoding = 'utf-8') as fi:
        finale = fi.read()
        print(finale)
    return len(parole_ordinate)
    ...
    for nome in os.listdir(root):
        ...
        if os.path.isdir(fullname):
            ...
            with open(fullname, 'r', encoding = 'utf-8') as file:
                testo = file.read()
                if testo[0] == testo[-1]:
                    insieme = dizionario.get(root, set())
                    insieme.add(nome)
                    dizionario[root] = insieme
...
def ex2(root):
    return esplora_albero(root, 0)
def esplora_albero(root, livello):
    if root is None:
        return ""
    N = root.value
    L = esplora_albero(root.left, livello+1)
    R = esplora_albero(root.right, livello+1)
    if livello % 2 == 0:
        return L + N + R
    else:
        return R + N + L
