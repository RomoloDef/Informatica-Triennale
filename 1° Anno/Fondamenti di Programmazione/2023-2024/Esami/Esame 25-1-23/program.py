...
def func2(pathname):
    ...
    with open(pathname, 'r') as file:
        ...
        for riga in righe:
            numero, matricola = riga.strip().split(',')
            if matricola in dizionario:
                if int(numero) > dizionario[matricola]:
                    dizionario[matricola] = int(numero)
            else:
                dizionario[matricola] = int(numero)
    return dizionario
    pass
...
def func3(listaA, pathname):
    def criterio_di_ordinamento(stringa):
        return len(stringa), stringa[::-1]
    listaA.sort(key=criterio_di_ordinamento)
    with open(pathname, 'w') as file:
        for stringa in listaA:
            file.write(stringa + '\n')
    return sum([len(stringa) for stringa in listaA]) 
    pass
...
def func4(S):
    ...
    for parola in testo.split():
        lista.append(''.join([c for c in parola if c.isalpha()]))
    lista.sort()
    dizionario = {}
    for valore in lista:
        dizionario[valore] = dizionario.get(valore, 0) + 1
    massimo = max([dizionario[valore] for valore in dizionario])
    istogramma = ''
    for valore in sorted(dizionario):
        spazi = ' ' * (massimo - dizionario[valore])
        istogramma += valore + spazi + '*' * dizionario[valore] + '\n'
    return istogramma
    pass
...
def func5(img, output_file_name):
    ...
    for y, riga in enumerate(immagine):
        for x, pixel in enumerate(riga):
            ...
            Y = x
            immagine[Y][X] = pixel
    images.save(immagine, output_file_name)
    return tuple(altezza, larghezza)
        ...
        if os.path.isfile(percorso):
            if nome.endswith('.txt'):
                with open(percorso, 'r') as file:
                    stringa = ''
                    for riga in file:
                        for numero in riga.strip().split():
                            stringa += chr(int(numero))
                dizionario[percorso] = stringa
        else:
            dizionario.update(ex1(percorso))
    return dizionario
    pass
