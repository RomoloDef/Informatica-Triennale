...
def ex1(root):
    # scrivi qui il tuo codice
    D = {}
    esplora_directory(root, D)
    return D

def esplora_directory(rootdir, dizionario):
    stringa = ''
    for nome in sorted(os.listdir(rootdir)):
        fullname = rootdir + '/' + nome
        if os.path.isdir(fullname):
            esplora_directory(fullname, dizionario)
        elif nome.endswith('.txt'):
            with open(fullname, encoding='utf8') as FIN:
                for s in FIN.read().split():
                    stringa += chr(int(s))
    if stringa:
        dizionario[rootdir] = stringa
...
def ex2(nums, ops):
    # scrivi qui il tuo codice
    if len(nums)==1:
        # X, *resto = nums        # prendo il valore in nums e il resto va in *resto (lista vuota)
        X = nums.copy().pop()     # prendo il valore unico contenuto in nums
        return {str(X)}
    risultato = set()
    for X in nums:
        resto = nums - {X}
        for op in ops:
            sottoespressioni = ex2(resto, ops)
            for sottoesp in sottoespressioni:
                nuova_espressione = str(X)+op+sottoesp
                risultato.add(nuova_espressione)
    return risultato
