...
def func2(a_dictionary):
    risultato = {}
    for chiave, valori in a_dictionary.items():
        print(valori)
        risultato[chiave] = sum(valori)
        print(risultato)
    chiave_max = max(risultato, key = risultato.get)
    return chiave_max
        
    pass

#print(func2({"a": [3, 2, 2], "b": [4, 2, 3], "c": [-4, 2, 2]}))
...
def func3(list_A, list_B):
    risultato = []
    for str_A, str_B in zip(list_A, list_B):
        str_A = str_A.lower()
        str_B = str_B.lower()
        common_chars = set(str_A) & set(str_B)
        common_chars_sorted = ''.join(sorted(common_chars))
        risultato.append(common_chars_sorted)
...
def esplora_dir(directory, dizionario):
    conteggio = 0
    for nome in listdir(directory):
        fullname = directory + '/' + nome
        if isdir(fullname):
            esplora_dir(fullname, dizionario)
        elif fullname.endswith('.txt'):
            conteggio += 1
    dizionario[directory] = conteggio
...
def ex2(T):
    ...
    if T.right is not None and T.left is not None:
        D += 1
    elif (T.right is not None and T.left is None) or (T.right is  None and T.left is not None):
        S += 1
    else:
        L += 1
    return L, S, D
