...
def func2(a_dictionary):
    risultato = {}
    for chiave, valore in a_dictionary.items():
        risultato[chiave] = sum(valore)
    chiavi_ordinate = sorted(risultato.keys())
    chiave_max = max(chiavi_ordinate, key = lambda k: risultato[k])
    return chiave_max
    pass
...
def func3(list_A, list_B):
    lista_finale = []
    lista = list(zip(list_A, list_B))
    for stringa_a, stringa_b in lista:
        stringa_a = stringa_a.lower()
        stringa_b = stringa_b.lower()
        caratteri = set(stringa_a) & set(stringa_b)
        caratteri_comuni = ''.join(sorted(caratteri))
        lista_finale.append(caratteri_comuni)
...
output_filename = 'func4/out_example1.txt'
#input_filename = 'func4/in_example2.txt'
#output_filename = 'func4/out_example2.txt'
...
def ex1(target_folder):
    d = {}
    esplora_dirin(target_folder, d)
    return sorted(d.items(), key = lambda x: (-x[1], x[0]))
    
    
def esplora_dirin(dirin, dizionario):
    conteggio = 0
    for nome in listdir(dirin):
        fullname = dirin + '/' + nome
        if isdir(fullname):
            esplora_dirin(fullname, dizionario)
        elif fullname.endswith('.txt'):
            conteggio += 1
    dizionario[dirin] = conteggio
