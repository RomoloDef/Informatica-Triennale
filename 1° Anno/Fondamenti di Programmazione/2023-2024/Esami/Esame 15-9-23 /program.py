...
def func1(int_list, keys):
    dizionario = {}
    for chiave in keys:
        interi_divisibili = list()
        for numero in int_list:
                if numero % chiave == 0:
                    interi_divisibili.append(numero)
        lista = list(interi_divisibili)
...
def func2(a_string, char):
    lista_caratteri = []
    for carattere in a_string:
        if ord(carattere) > ord(char):
            lista_caratteri.append(carattere)
    lista_caratteri = list(set(lista_caratteri))
    lista_caratteri.sort(reverse = True)
    nuova_stringa = ''.join(lista_caratteri)
    ...
    with open(input_file, 'r', encoding = 'utf-8') as file:
        ...
        for riga in righe:
            ...
            for parola in riga.split():
                prodotto *= sum([int(c) for c in parola if c.isdigit()])
            prodotti.append(prodotto)
        for prodotto in prodotti:
            somma += prodotto
    with open(output_file, 'w', encoding = 'utf-8') as f:
        print(prodotto, file = f)    
    return somma
    pass
    ...
    if n > len(a_set):
        ...
        for carattere in a_set:
            ...
            for sottosoluzione in sottosoluzioni:
                risultato.add(carattere + sottosoluzione)
        return risultato
    pass
...
def ex2(root, m, n, k, level=0):
    return conto_nodi(root, m, n, k, level )
def conto_nodi(root, m, n, k, level):
    if root is None:
        return 0
    conteggio = 0
    if m <= root.value <= n and root.value % k == 0:
        conteggio += root.value
    conteggio += conto_nodi(root.letf, m, n, k, level + 1)
    conteggio += conto_nodi(root.right, m, n, k, level + 1)
    return conteggio
    pass
