...
def func1(lists):
    intersezione = set(lists[0])
    
    for lista in lists[1:]:
        intersezione = intersezione.intersection(set(lista))
    
    lista_finale = list(intersezione)

    lista_finale.sort(reverse=True)
...
def func2(text):
    ...
    for char in text:
        if char.isdigit():  # Controlla se il carattere è una cifra
            current_number += char  # Aggiunge la cifra al numero corrente
        else:
            if current_number:  # Se abbiamo un numero corrente
                max_number = max(max_number, int(current_number))  # Aggiorna il numero massimo
                current_number = ''  # Resetta il numero corrente

    # Verifica l'ultimo numero dopo aver finito di scorrere la stringa
    if current_number:
...
def func3(textfile_in, textfile_out):
    somma_pari = 0
    somma_dispari = 0
    with open(textfile_in, 'r', encoding='utf-8') as file:
        with open(textfile_out, 'w', encoding='utf-8') as f:
            righe = reversed(file.readlines())
            for riga in righe:
                numeri = riga.split(',')
                prodotto_numeri_pari = 1
                prodotto_numeri_dispari = 1
                for numero in numeri:
                    if int(numero) % 2 == 0:
                        prodotto_numeri_pari *= int(numero)
                        somma_pari += int(numero)
                    else:
                        prodotto_numeri_dispari *= int(numero)
                        somma_dispari += int(numero)
                differenza = prodotto_numeri_pari - prodotto_numeri_dispari
                f.write(f'{differenza}\n')
            ...
            if os.path.isdir(fullname):
                ...
                for ext in extensions:
                    if fullname.endswith(ext):
                        file_sizes.append(os.path.getsize(fullname))
                        break
        
        if file_sizes:
            dizionario[directory] = [min(file_sizes), max(file_sizes)]
