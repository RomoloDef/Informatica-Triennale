...
def func1(lists):
    ...
    lista_finale = sorted(lista_finale, reverse = True)
    return lista_finale
    pass
...
Definisci la funzione func2(text) che riceve come argomento:
- text: una stringa di testo
e che restituisce il valore del numero più grande (sequenza di cifre) trovato nel testo.

Esempio:
text = 'sotto la panca 1234Il cane3212Sta Cantando 4S5oV6e7r8t LA PANCA IL CANE muore'
expected = 3212
'''
...
def func2(text):
    ...
    # Controlla l'ultimo numero trovato
    if current_num:
        num = int(current_num)
        if max_num is None or num > max_num:
            max_num = num
