...
def func1(list_a, list_b, list_c):
    risultato = []
    for stringa in list_a:
        if stringa in list_b and stringa in list_c:
            risultato.append(stringa)
    risultato = list(set(risultato))
    return sorted(risultato, key = lambda parola: parola)
    pass
...
def func2(d1, d2):
    ...
    for chiave in d1.keys():
        if chiave in d2.keys():
            d3[chiave] = d1[chiave] + d2[chiave]
        ...
        for x in range(larghezza):
            if img[y][x] == (255, 255, 255):  
                ...
                if x_start is not None:      
                    segmenti.append((y, x_start, x-1))
                    x_start = None
        if x_start is not None:
            segmenti.append((y, x_start, larghezza-1))
        ...
        if os.path.isdir(fullname):
            ...
            with open(fullname, 'r', encoding = 'utf-8') as file:
                ...
                for riga in righe:
                    ...
                    risultato[fullname] = max(risultato.get(fullname, 0), lunghezza)
    return sorted(risultato.items(), key = lambda x: (x[1], x[0]))
...
def esplora_albero(root, livello, dizionario):
        if root is None:
            return ""
        if livello in dizionario:
            dizionario[livello] += root.value
        else:
            dizionario[livello] = root.value
        esplora_albero(root.left, livello+1, dizionario)
        esplora_albero(root.right, livello+1, dizionario)
