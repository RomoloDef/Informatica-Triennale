...
def func1(list_a, list_b, list_c):
    ...
    for parola in list_a:
        if parola in list_b and parola in list_c:
          lista_finale.append(parola)
    criterio_di_ordinamento = lambda parola: parola
    lista_finale = sorted(set(lista_finale), key=criterio_di_ordinamento, reverse=False)
    return lista_finale
    pass
...
def func2(d1, d2):
    ...
    chiavi1 = d1.keys()
    chiavi2 = d2.keys()
    for chiave in chiavi1:
        if chiave in chiavi2:
            d3[chiave] = d1[chiave] + d2[chiave]
    d3 = dict(sorted(d3.items(), key=lambda x: x[0]))
    return d3
    pass
...
def func5(input_pngfile):
    immagine = images.load(input_pngfile)
    segmenti = []
...
def ex1(directory):
    ...
    def criterio_di_ordinamento(coppia):
        return coppia[1], coppia[0]
    return sorted(risultato.items(), key=criterio_di_ordinamento)
    pass
