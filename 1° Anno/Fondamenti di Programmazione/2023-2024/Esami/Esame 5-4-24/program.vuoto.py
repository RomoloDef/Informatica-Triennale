...
def ex2(root):
    return esplora_directory(root, 0)
    
def esplora_directory(root, profondità):
    if root is None:
        return 0
    else:
        count = 0
        if profondità % 2 == 0:
            ...
            if root.right is not None:
                count += 1
    return count + esplora_directory(root.right, profondità+1) + esplora_directory(root.left, profondità+1)
