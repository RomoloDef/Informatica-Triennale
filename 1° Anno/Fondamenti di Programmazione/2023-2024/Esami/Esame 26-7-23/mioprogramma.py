...
def func1(diz1, diz2):
    ...
    for chiave, valore in diz1.items():
        if chiave in diz2:
            diz3[chiave] = sorted(list(set(valore) ^ set(diz2[chiave])), key=lambda x: (-len(x), x))
    return diz3
...
def func2(text):
    dizionario = {}
    for stringa in text.split():
        if stringa[0].lower() in dizionario:
            dizionario[stringa[0].lower()] += 1
        else:
            dizionario[stringa[0].lower()] = 1
    return dizionario
...
def func3(textfile_in, textfile_out):
    with open(textfile_in, 'r', encoding = 'utf-8') as file:
        ...
        numeri = testo.split()
    numeri_ordinati = sorted(numeri, key=lambda x: (len(x), float(x)))
    with open(textfile_out, 'w', encoding = 'utf-8') as f:
        testo_scritto = f.write(', '.join(numeri_ordinati))
    return len(numeri)
...
def func4(input_filename):
    ...
    with open(input_filename, 'r', encoding = 'utf8') as file:
        for riga in file:
            matrice.append(list(map(int, riga.strip().split())))
    trasposta = []
    for i in range(len(matrice[0])):
        trasposta.append([riga[i] for riga in matrice[::-1]])
    trasposta = trasposta[::-1]
    return trasposta
    pass
...
def ex1(root : NaryTree, valori : list[int]):
    return funzione_ricorsiva(root, valori, 0)

def funzione_ricorsiva(root : NaryTree, valori : list[int], profondità : int):
        if profondità < len(valori):
            root.value += valori[profondità]
        total = root.value
        for son in root.sons:
            total += funzione_ricorsiva(root, valori, profondità +1)
        return total
...
def ex2(dirin, words):
    ...
    for word in words:
        dizionario[word] = 0
    esplora_dirin(dirin, words, dizionario)
    return sorted(dizionario.items(), key=lambda item: (-item[1], item[0]))

def esplora_dirin(dirin, words, D):
    for nome in os.listdir(dirin):
        ...
        if os.path.isfile(fullname) and fullname.endswith(".txt"):
            ...
            esplora_dirin(fullname, words, D)
