...
def func4(input_filename):
    ...
    with open(input_filename, 'r', encoding = 'utf-8') as f:
        ...
        for riga in righe:
            matrici = []
            for numero in riga.split():
                if numero[-1]:
                    matrici.append(numero)
                print(matrici)
...
def ex2(dirin, words):
    dizionario = {}
    esplora_dirin(dirin, 0, words, dizionario)
    return sorted(dizionario.items(), key = lambda parola: (-len(parola), parola), reverse = True)
    
def esplora_dirin(dirin, words, profondità, D):
        for nome in os.listdir(dirin):
            ...
            if os.path.isfile(fullname) and nome.endswith(".txt"):
                ...
                esplora_dirin(fullname, profondità+1, words, D)
