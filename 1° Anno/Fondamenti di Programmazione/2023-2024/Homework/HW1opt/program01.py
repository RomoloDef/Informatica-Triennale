...
def ex1(int_seq, subtotal):
    int_seq = list(map(int, int_seq.split(',')))  # Converti la stringa in una lista di numeri
    cumulative_sum = [0]  # Lista per le somme cumulative
    count = 0  # Contatore per il numero di sottostringhe con somma uguale a subtotal

    # Calcola le somme cumulative
    for num in int_seq:
        cumulative_sum.append(cumulative_sum[-1] + num)

    # Verifica quante volte appare subtotal nelle somme cumulative
    for i in range(len(cumulative_sum)):
        for j in range(i + 1, len(cumulative_sum)):
            if cumulative_sum[j] - cumulative_sum[i] == subtotal:
                count += 1
