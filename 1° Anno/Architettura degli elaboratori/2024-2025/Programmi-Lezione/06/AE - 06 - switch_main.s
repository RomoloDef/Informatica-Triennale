# Programma ASM (assembly RISC-V) che implementa lettura di valori da tastiera da utente e 
...
# le info sulla eccezione sono nei registro CS

.globl main

.data
	switch_case:  .word case_00, case_01
	case_string:  .asciz "Ran case #: "
	input_question: .asciz "Input 0 for sum or 1 for multiplication\n"
	address_of_forbidden: .word forbidden #for example of exploit
	hidden_text: .asciz "forbidden!"

.text
main:
	jal read_int # Leggi intero
	mv t2,a0 # mettilo in t2
	
	jal read_int # Leggi intero
	mv t3,a0 # mettilo in $t3
	...
	mv  t1,a0
	slli t0, t1, 2    				# the index of the case to run points to the address of the line of code
	                   				# so I have to index the mem at location switch_case. Given I have word,
	                   				# the offset is 0,4,8,12...
	la t4, switch_case
	add t4, t4, t0
	lw s1, (t4)  		# load the corresponding address of case

	la a0, case_string
	jal print_string        # Print first part of case selected
	
	mv a0,t1             	# Print 0 or 1 case selected
	jal print_rez
